...
def img_folder_trim(img_folder, pattern):

    """
    For an image folder, remove all images after the pattern appear. This is typical useful if the imaging goes off for super long time. 
    ...
    files = folder_file_num(img_folder)
    ...
    while i<len(files):
        os.remove(files[i])
        i = i+1
    print("File removal is complet.")

    
def folder_img_remove(folder, pattern):
    """Remove by pattern"""
    folder = folder_verify(folder)
    img_paths = folder_file_num(folder, pattern)
    
    for f in img_paths:
        os.remove(f)
    
    print("All %s images are removed!"%pattern)
