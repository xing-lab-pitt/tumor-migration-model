...
def read_time_series(start, end, interval, data_folder, rep_inx, iter_inx):
    ...
    for t in t_list:
        ...
        px = past_df.x_self_polarity
        py = past_df.y_self_polarity
        fx = -past_df.f_x
        fy = -past_df.f_y
    ...
    past_px = np.array(past_px).T
    past_py = np.array(past_py).T
    past_fx = np.array(past_fx).T
    past_fy = np.array(past_fy).T
    past_angle = np.array(past_angle).T
...
def polarity_mean(past_px, past_py):
    ...
    # mean for all cells
    px_mean = px_time_mean.mean()
    py_mean = py_time_mean.mean()
    # mean polarity
    p_mean = (px_mean**2+py_mean**2)**0.5
    #print(p_mean)
    
    # variance 1, tumor variance then average over time
    px_cell_mean = past_px.mean(axis = 0, keepdims=True)
    py_cell_mean = past_py.mean(axis = 0, keepdims=True)
    tumor_p_var = (((past_px-px_cell_mean)**2+(past_py-py_cell_mean)**2).mean(axis=0).mean())**0.5
    
    # variance 2, variance over time then average for each cell.
    cell_p_var = (((past_px-px_time_mean)**2+(past_py-py_time_mean)**2).mean(axis=1).mean())**0.5
    
    return (p_mean, tumor_p_var, cell_p_var)
...
def tumor_geo_calc(data_folder, feature_folder, figure_folder,
    ...
    (p_mean_180, tumor_p_var_180, cell_p_var_180) = polarity_mean(past_px, past_py)
    
    print("p_mean: %s, tumor_p_var: %s, cell_p_var: %s"%(p_mean_180, tumor_p_var_180, cell_p_var_180))
    
    #-----------------------Final 30 frame
    (past_px, past_py, past_fx, past_fy, past_angle) = read_time_series(240, 270, 5, 
                                                                        data_folder, rep_inx, iter_inx)
    #print("shape is: ", past_px.shape)
    (p_mean_30, tumor_p_var_30, cell_p_var_30) = polarity_mean(past_px, past_py)
    
    print("p_mean: %s, tumor_p_var: %s, cell_p_var: %s"%(p_mean_30, tumor_p_var_30, cell_p_var_30))
...
#     "norm_moment": norm_moment,
    ...
    "tumor_p_var_180":tumor_p_var_180,
    "cell_p_var_180":cell_p_var_180,

    "p_mean_30":p_mean_30,
    "tumor_p_var_30":tumor_p_var_30,
